    ...
    match parse_expr.relation {
        Some(rel) => {
            match rel.0 {
                RelOp::Equal => {
                    Expr::BinOp(
                        Box::new(expr_from_simple(parse_expr.simple_expr)),
                        BinOp::Equal,
                        Box::new(expr_from_simple(rel.1)),
                    )
                }
                RelOp::NEqual => {
                    Expr::BinOp(
            ...
            match simple.term_chain.pop() {
                Some(term_c) => {
                    match term_c.0 {
                        AddOp::Add => {
                            Expr::BinOp(
                                Box::new(expr_from_term(term_c.1)),
                                BinOp::Add,
                                Box::new(expr_from_simple(simple)),
                            )
                        }
                        AddOp::Sub => {
                            Expr::BinOp(
    ...
    match expr {
        Expr::Constant(c) => {
            match c {
                ...
                Constant::Int(i) => Expr::Constant(Constant::Int(i.checked_neg().expect(
                    "Cannot make such a large number negative",
                ))),
            }
        }
    ...
    match term.fac_chain.pop() {
        Some(fac_ch) => {
            match fac_ch.0 {
                MulOp::Mul => {
                    Expr::BinOp(
                        Box::new(expr_from_factor(fac_ch.1)),
                        BinOp::Mul,
                        Box::new(expr_from_term(term)),
                    )
                }
                MulOp::Div => {
                    Expr::BinOp(
